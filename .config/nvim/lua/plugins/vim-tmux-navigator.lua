return {
  {
    ...
    keys = {
      { "<M-Left>", desc = "Smart Navigate Left" },
    ...
    config = function(_, opts)
      ...
      -- Setup our custom navigation after plugin loads
      ...
      vim.keymap.set('n', '<M-Up>', function()
        ...
        -- If we didn't move (same window), try tmux session up
        if current_win == original_win then
          vim.fn.system('~/.config/tmux/scripts/tmux_sessions.sh prev')
        end
      ...
      vim.keymap.set('n', '<M-Down>', function()
        ...
        -- If we didn't move (same window), try tmux session down
        if current_win == original_win then
          vim.fn.system('~/.config/tmux/scripts/tmux_sessions.sh down')
        end
